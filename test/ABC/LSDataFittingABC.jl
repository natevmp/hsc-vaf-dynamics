...
SCBurdenHSC_CID = mutBurden_col[HSCMask_col]

SCBurdenHSCMean = mean(mutBurden_col[HSCMask_col])
SCBurdenHSCVar = var(mutBurden_col[HSCMask_col])


println(SCBurdenHSCMean)
println(SCBurdenHSCVar)
println(SCBurdenHSCVar/SCBurdenHSCMean - 1)
...
@save "data/LSDataStats.jld2" SCBurdenHSC_CID SCBurdenHSCMean SCBurdenHSCVar freqs_f nVHSC_f sampleSize
## ============================== ABC pipeline ==============================
...
dataParams = Dict(
    "evolveTime" => agePatient,
    "sampleSize" => sum(HSCMask_col),
    "mutMean" => SCBurdenHSCMean,
    "mutVar" => SCBurdenHSCVar,
    "μ" => 1.2,
    "λPar" => SCBurdenHSCMean / 1.2
)
...
@time simResult = runabc(setup, nVHSC_f, verbose=true, progress=true, parallel=true)
